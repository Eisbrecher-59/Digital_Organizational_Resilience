...
\tiny
\begin{longtblr}[
  caption = {\textbf{Legacy classification of resilience-related publications by discipline and methodology.} Original screening of the corpus, kept here as an archival record of how the literature distributes across academic traditions (rows) and analytical approaches (columns). Red cells mark research gaps where no publication was identified; orange cells indicate sparse coverage; grey cells denote intersections judged less relevant to the present review. The analytical version used in the paper -- restructured by method and coordination scope to align with the Enactment--Scope framework -- appears in Table~\ref{tab:ResearchGaps} (Section~\ref{sec:RG}).},
  label = {tab:ResearchGapsLegacy}
]{
  width=\linewidth,
