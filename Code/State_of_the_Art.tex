...
% Set the natural exported figure width here (used when including without resize).
\setlength{\FigureWidth}{16cm}
\pgfmathsetlengthmacro{\FigureUnit}{\FigureWidth/15.2}
\begin{document}

\begin{tikzpicture}[x=\FigureUnit,y=\FigureUnit,font=\rmfamily, line cap=round, line join=round]
...
\tikzset{
  ...
  clabel/.style={font=\bfseries,align=center},
...
\coordinate (Fsw) at (-7.80,-5.55);
\coordinate (Fne) at ( 7.4, 5.25);
\coordinate (Fnw) at (-7.80, 5.25);
\coordinate (Fse) at ( 7.4,-5.55);
...
\node[box, draw=none, fill=white, text=black, font=\bfseries\Large, inner sep=3pt]
  at (0,5.20) {From articulated Resilience to enacted Capability};

% ---------- main triangle ----------
\draw[tri] (L) -- (Apex) -- (R) -- cycle;

% Vertex labels
\node[clabel] at ($(Apex)+(0,0.50)$) {Theory};

\node[clabel] at ($(L)+(-1.55,-0.75)$)
  {Measurement\\ \&\\ Modeling};

\node[clabel] at ($(R)+(1.4,-0.75)$)
  {Governance\\ \&\\ Design};

% ---------- side mechanism boxes ----------
% Left side (rotated along left edge)
\node[box, rotate=61, text width=4.55cm, inner sep=2.0pt]
  at ($ (L)!0.5!(Apex) $)
...
% ---------- center box ----------
\node[box, text width=4.55cm, inner sep=3.5pt]
  at (0, -1.75)
...
% ---------- bottom box ----------
\node[box, text width=4.55cm, inner sep=2.0pt]
  at (0, -3.85)
