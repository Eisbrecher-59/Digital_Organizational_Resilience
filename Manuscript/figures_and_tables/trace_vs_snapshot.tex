...
% ---- endpoint labels (outcome) ----
\node[green!55!black,right,font=\scriptsize] at (9.6,4.75) {stronger};
\node[blue!45!black,right,font=\scriptsize]  at (9.6,3.0)  {unchanged};
\node[red!75!black,right,font=\scriptsize]   at (9.6,1.6)  {more brittle};
...
What diverges is the recovery: where monitoring feeds a systematic
post-incident learning loop, each shock is converted into a net gain and
capacity rises (\emph{stronger}); where organizations respond but institutionalize
nothing, capacity returns to baseline (\emph{unchanged}); and where no learning loop
exists, lessons are lost and each shock leaves the organization measurably more
brittle (\emph{more brittle}). Only tracing this loop across successive disruptions
makes the difference---and thus resilience versus luck, capability versus
